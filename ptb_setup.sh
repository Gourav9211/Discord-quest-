...
setup() {
    ...
    BGR_URL="https://raw.githubusercontent.com/xsyanic/discord-quest-mobile/main/bgr.jpg"
    BGR_PATH="$HOME/Pictures/bgr.jpg"
    mkdir -p "$HOME/Pictures"
    wget -O "$BGR_PATH" "$BGR_URL"
    chmod 644 "$BGR_PATH"
    mkdir -p ~/.config/openbox
    echo "feh --bg-fill $BGR_PATH &" >> ~/.config/openbox/autostart
    ...
    echo "### SU: Setting DNS to Google DNS..."
    sudo bash -c 'echo -e "nameserver 8.8.8.8\nnameserver 8.8.4.4" > /etc/resolv.conf'
    
    # Make resolv.conf immutable to prevent resets
    sudo chattr +i /etc/resolv.conf

    echo "### SU: DNS successfully set to Google DNS."
